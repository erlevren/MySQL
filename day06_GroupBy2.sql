...
select * from personel group by ulke; -- bu sorgulama yanlş olur. group by ile tabloda geçen ayn isimleri alrz.
...
select ulke, count(*) as calisan_sayisi from personel
group by ulke;


-- SORU04: Maas ortalamasını bayanlar ve baylar olarak sorgulayınız
select cinsiyet, round(avg(maas),2) as ortalama_maas from personel
group by cinsiyet;
...
select ulke, sehir, count(*) as calisan_sayisi from personel group by ulke, sehir;


-- SORU07: Her ulke için bay ve bayan çalışan sayısı ve yaş ortalamasını sorgulayınız.
select ulke, cinsiyet, count(*) as calisan_sayisi, round(avg(yas)) as ortalama_yas from personel
group by ulke, cinsiyet;

select ulke, cinsiyet, count(*) as calisan_sayisi, round(avg(yas),2) as ortalama_yas from personel 
group by ulke,cinsiyet
order by ulke asc;
...
select ulke, cinsiyet, count(*) as kisi, round(avg(yas), 2) as ortalama_yas from personel where maas>30000
group by ulke, cinsiyet;
...
select isim, urun_adi, sum(urun_miktari) as toplam_miktar from manav where urun_adi is not null
...
select isim, urun_adi, sum(urun_miktari) as tplm_urun from manav
group by isim, urun_adi
having tplm_urun >=3 -- aggregate fonksiyonlar ile ilgili bir koşul koymak için group by dan sonra having kullanılır.
order by tplm_urun desc;
-- where kullanamayiz cunku sum(urun_miktari) as toplam_urun aggregate fonksiyondan geliyor
...
group by urun_adi
having max(urun_miktari) <> min(urun_miktari)	-- <> yerine != kullanailiriz
order by max(urun_miktari);


select isim,urun_adi, max(urun_miktari) from manav 
group by urun_adi
having max(urun_miktari) <> min(urun_miktari) -- <> demek != demektir
order by max(urun_miktari);
-- Windowsta çalşyor.
-- mAC'TE ÇALŞMYOR
...
select urun_adi, count(*) from manav where urun_adi is not null group by urun_adi;


-- Satılan farklı meyve türlerinin sayısını listeleyen sorgu
select count(distinct urun_adi) as kac_urun from manav;

-- satılan meyve + isimleri farklı olanları listeleyen sorgu
select distinct urun_adi, isim from manav where urun_adi is not null order by urun_adi;
